...
class EosAccountRequestImpl @Inject constructor(
    ...
    override fun getAccount(accountName: String): Single<Result<EosAccount, AccountError>> {
        return chainApi.getAccount(AccountName(accountName))
            ...
            if (it.isSuccessful) {
                val account = it.body()!!
                Result(EosAccount(account.account_name))
            } else {
                Result<EosAccount, AccountError>(AccountError.FailedRetrievingAccount(
                    it.code(),
                    it.errorBody()
                ))
            }
